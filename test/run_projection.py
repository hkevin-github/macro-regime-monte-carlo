...
def check_prerequisites():
    required_files = [
        ...
        "data/models/regime_labels.json",
        "data/models/transition_matrix.csv",
    ]
...
def create_visualizations(regime_results, baseline_results):
    ...
    ax1.hist(regime_finals, bins=60, alpha=0.6, color='blue', edgecolor='black', label='Regime-Aware')
    ax1.hist(baseline_finals, bins=60, alpha=0.6, color='green', edgecolor='black', label='Baseline')
    ax1.axvline(np.median(regime_finals), color='blue', linestyle='--', linewidth=2, label=f'Regime Median: ${np.median(regime_finals):,.0f}')
    ax1.axvline(np.median(baseline_finals), color='green', linestyle='--', linewidth=2, label=f'Baseline Median: ${np.median(baseline_finals):,.0f}')
    ax1.set_xlabel('Final Balance ($)')
    ax1.set_ylabel('Frequency')
    ax1.set_title(f'Distribution of Outcomes ({N_YEARS} Years)', fontweight='bold')
    ...
    ax2.set_title('Percentile Comparison', fontweight='bold')
    ax2.set_xticks(x)
    ...
    n_paths_to_plot = min(100, len(regime_results['results']))
    ...
    ax3.axhline(CLIENT_SCENARIO.initial_balance, color='black', linestyle='--', alpha=0.5, label='Initial Balance')
    ax3.axhline(0, color='red', linestyle='--', alpha=0.5, label='Depletion')
    ax3.set_xlabel('Years')
    ax3.set_ylabel('Portfolio Balance ($)')
    ax3.set_title('Balance Evolution (Regime-Aware)', fontweight='bold')
...
def save_detailed_results(regime_results, baseline_results):
    ...
    print("[+] Summary saved to: test/output/summary_comparison.csv")
    ...
    print("[+] Regime trial results saved to: test/output/regime_trial_results.csv")
    ...
    print("[+] Baseline trial results saved to: test/output/baseline_trial_results.csv")
